...
**最失望的**：68 篇博客里几乎没有人在认真讨论 Agent-to-Agent 通信。Cloudflare 和 Stripe 在构建 Agent-to-Platform 协议，但 Agent 之间怎么协商、信任、解决冲突？几乎空白。这暗示多 Agent 系统的真实需求可能远小于宣传。

**最一致的信号**：从 Chip Huyen（2025-01）到 Braintrust（2026-06），几乎所有独立来源在最底层认知上是一致的——产品设计 > 模型能力，上下文工程 > 模型选择，框架 > 模型。18 个月，68 篇博客，不同的作者和机构，全部收敛到同一个方向。
