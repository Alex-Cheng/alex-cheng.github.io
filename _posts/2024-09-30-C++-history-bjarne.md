...
C++标准是ISO标准，制定C++标准的组织是ISO国际C++标准委员会，正式名称为ISO/IEC JTC1/SC22/WG21。
...
除了上述介绍的新的并发支持的标准库组件，C++11还引入了regex、chrono、random组件，以及基于哈希表的关联容器（C++98的关联容器是基于红黑树的）。更多信息通过查阅[cppreference.com](https://en.cppreference.com/w/)了解更多。
...
例如我要写一个模板函数，接收模板类型参数T，希望T类型能够有移动构造函数。那么我可以写成：
